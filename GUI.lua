...
local DAY_CELL_WIDTH  = 120
local DAY_CELL_HEIGHT = 90
...
if not Ahoydar.uiFrame then
    ...
    -- Кнопка закрыть (возвращена в правый верхний угол)
    ...
    Ahoydar.monthLabel:SetText("Месяц Год")
        ...
        for row = 1, NUM_ROWS do
            for col = 1, NUM_COLUMNS do
                ...
                cell:SetScript("OnEnter", function(self)
                    self:SetBackdropColor(0.9, 0.9, 0.1, 0.4)
                ...
                cell:SetScript("OnLeave", function(self)
                    if self.isToday then
                        self:SetBackdropColor(0, 0.75, 1, 0.3)
                    else
...
function Ahoydar:UpdateCalendar()
    ...
    self.monthLabel:SetText(date("%B %Y", monthTime))
    ...
    for i = startIndex, startIndex + daysInMonth - 1 do
        ...
        if cellKey == todayKey then
            ...
            cell:SetBackdrop(commonBackdrop)
            cell:SetBackdropColor(0, 0.75, 1, 0.3)
...
function Ahoydar:FillDayCellWithEvents(cell, eventsForDay)
    ...
    for i, eventData in ipairs(eventsForDay) do
        if offsetY > (cell.eventsFrame:GetHeight() - 20) then break end
        local iconFrame = CreateFrame("Frame", nil, cell.eventsFrame, "BackdropTemplate")
        iconFrame:SetSize(cell.eventsFrame:GetWidth(), 20)
        iconFrame:SetPoint("TOPLEFT", 0, -offsetY)
        iconFrame:SetBackdrop({
            bgFile = "Interface\\Buttons\\WHITE8x8",
            edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
            edgeSize = 8,
        })
        iconFrame:SetBackdropColor(0.2, 0.6, 0.2, 0.8)
